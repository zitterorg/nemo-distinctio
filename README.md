# @zitterorg/nemo-distinctio

![CI](https://github.com/zitterorg/nemo-distinctio/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@zitterorg/nemo-distinctio.svg?style=flat)](https://www.npmjs.com/package/@zitterorg/nemo-distinctio)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)


`@zitterorg/nemo-distinctio` enables the use of [CORS](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) in a Fastify application.

## Install
```
npm i @zitterorg/nemo-distinctio
```
...
## Usage
Require `@zitterorg/nemo-distinctio` and register it as any other plugin, it will add an `onRequest` hook and a [wildcard options route](https://github.com/fastify/fastify/issues/326#issuecomment-411360862).
```js
import Fastify from 'fastify'
import cors from '@zitterorg/nemo-distinctio'
...
fastify.register(require('@zitterorg/nemo-distinctio'), (instance) => {
  return (req, callback) => {
    const corsOptions = {
...
By default, `@zitterorg/nemo-distinctio` adds a `onRequest` hook where the validation and header injection are executed. This can be customized by passing `hook` in the options. Valid values are `onRequest`, `preParsing`, `preValidation`, `preHandler`, `preSerialization`, and `onSend`.
...
fastify.register(require('@zitterorg/nemo-distinctio'), {
  hook: 'preHandler',
